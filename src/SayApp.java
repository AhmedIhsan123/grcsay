...
/**
 * The SayApp class represents a simple application where users can choose an
 * animal
...
public class SayApp {
  ...
  /**
   * The main method is the entry point of the application. It handles the user
   * input
   * for selecting an animal and a message, then displays the speech bubble and
   * the
  ...
  public static List<Animal> animalList() {
    return Arrays.asList(new Cow(), new Duck(), new Camel());
  }

  /**
   * Maps animal names (converted to lowercase) to their corresponding Animal
   * objects.
   *
   * @param animals A list of Animal objects.
   * @return A map where the keys are animal names (in lowercase) and the values
   *         are the corresponding Animal objects.
   */
